const API = "52e7991c78ed41509d8ceb9918fa892d";
...
const getCitiesUsingGeolocation = async(searchText) => {
    const response = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${searchText}&limit=5&appid=${API}`);
    return response.json()
}

const getCurrentWeatherData = async({lat, lon, name:city}) => {
    const url = lat && lon? `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API}&units=metric` : `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API}&units=metric`
...
const loadHourlyForecast = ({main:{temp: tempNow}, weather:[{icon: iconNow}]} ,hourlyForecast) =>  {
    ...
    <p class="hourly-temp"ß>${formatTemprature(tempNow)}</p>
    </article>`;
    ...
    for(let {temp, icon, dt_txt} of dataFor12Hours) {
        ...
        <p class="hourly-temp"ß>${formatTemprature(temp)}</p>
    </article>`
    }
...
const loadFiveDayForescast = (hourlyForecast) => {
    console.log(hourlyForecast)
...
const loadData = async() =>{
    ...
    loadFiveDayForescast(hourlyForecast)
